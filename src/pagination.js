    ...
    link: function($scope) {
      ...
      $scope.selectPage = function(pageNo) {
        ...
        if ($scope.vm.currentPage !== prePage) {
           ...
           * 调用回调方法
           * @param curPage 当前页码
           * @param prePage 上次点击的页码
           * @param start json数据起始索引
           * @param end json数据终止索引
           */
          ($scope.onChange || angular.noop)({
            curPage: $scope.vm.currentPage,
            prePage: prePage,
            start: $scope.vm.start,
            end: $scope.vm.end
          });
        }
      };
      ...
      function refresh() {
        ...
        var allPages = $scope.vm.totalPage;
        var toNumber;
        var number = ($scope.vm.currentPage - 1) * option.pageSize;
        if (
          allPages === $scope.vm.currentPage &&
          $scope.totalItems < option.pageSize * allPages
        ) {
          if ($scope.vm.currentPage * option.pageSize < $scope.totalItems) {
            toNumber =
              number +
              Math.abs(
                $scope.totalItems - $scope.vm.currentPage * option.pageSize
              );
          } else {
            toNumber = $scope.totalItems;
          }
        } else if (allPages < $scope.vm.currentPage) {
          number = (allPages - 1) * option.pageSize;
          toNumber = $scope.totalItems;
        } else {
          toNumber = number + option.pageSize;
        }
        $scope.vm.start = number;
        $scope.vm.end = toNumber;
        $scope.vm.pageCount = $scope.vm.currentPage;
        $scope.vm.pageList = generatePageList(
      ...
      function parseOption(option) {
        option.pageSize = option.pageSize
          ? parseInt(option.pageSize)
          : defaultOptions.pageSize;
        option.displayCount = option.displayCount
          ? parseInt(option.displayCount)
          : defaultOptions.displayCount;
        option.disableCounting =
          option.disableCounting !== undefined &&
          option.displayCount !== 'false';
